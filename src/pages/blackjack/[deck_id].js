...
import { drawCardsFromDeck } from "../../app/services/blackjack";
...
function App(initialState) {
  ...
  const handleClick = (e) => {
    e.preventDefault();
    window.location.assign("/blackjack/new");
  }
...
export const getServerSideProps = wrapper.getServerSideProps(
  (store) => async ({ query }) => {
    ...
    const { data: { cards } } = await store.dispatch(drawCardsFromDeck.initiate({ deckId, count: 4 }));
